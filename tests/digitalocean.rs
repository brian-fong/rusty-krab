...
mod digitalocean {
    ...
    async fn valid_fields() {
        ...
        if let Ok(lines) = read_file("./experimental/digitalocean_connection.txt") {
            for line in lines.flatten() {
                let parameter: Vec<&str> = line.split(":").map(|s| s.trim()).collect();
                if parameter.len() != 2 {
                    panic!("Invalid configuration for Digital Ocean database!");
                } else {
                    config.insert(parameter[0].to_string(), parameter[1].to_string());
                }
            }
        }
        ...
        // Read subscription
        let res_read = sqlx::query!("SELECT email, name FROM subscriptions")
            .fetch_all(&pool)
            .await
